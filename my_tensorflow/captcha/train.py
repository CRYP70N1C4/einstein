...
class Discuz():
    def __init__(self):
        ...
        # 测试集大小
        self.test_size = 500000
        # 每次获得batch_size大小的当前训练集指针
        self.train_ptr = 0
        # 每次获取batch_size大小的当前测试集指针
        self.test_ptr = 0
        # 字符字典大小:0-9 a-z A-Z _(验证码如果小于4，用_补齐) 一共63个字符
        self.char_set_len = 20
        # 验证码最长的长度为4
        self.max_captcha = 6
        # 输入数据X占位符
        self.X = tf.placeholder(tf.float32, [None, 100*35])
        # 输入数据Y占位符
        self.Y = tf.placeholder(tf.float32, [None, 6*20])
        # keepout占位符
    ...
    def get_next_batch(self, train_flag=True, batch_size=100):
        ...
        return helpers.get_next_batch(batch_size)
